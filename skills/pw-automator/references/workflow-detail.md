...
## Step 7: 生成最终脚本 + 询问定时执行

### 生成最终脚本

应用步骤 6 中所有确认的替换方案，生成完整的优化脚本。确保:

- 所有语义定位器替换已应用
- 所有 wait_for_timeout → expect/loop 替换已应用
- 所有模式增强（iterate/branch/loop）已应用
- pw-kit 函数调用已添加（extract_download_urls、click_with_offset 等）
- 容错处理已添加（try/except + screenshot）

### 询问定时执行

脚本生成后，**主动询问用户是否需要定时执行该脚本**:

```
✅ 优化脚本已生成: my_script_optimized.py
...
如果需要，我将使用 schedule_run() 生成 cron 或 daemon 配置。
```

⚠️ `schedule_run` 不从脚本分析中发现 — 它只在用户明确需要定时执行时才使用。步骤 7 是唯一主动提出这个选项的时刻。

用户确认需要定时执行后，根据用户选择的时间模式:
...
# cron 模式（服务器推荐）
config = schedule_run(
    script_path="my_script_optimized.py",
    time="09:00",
    mode="cron",
)
...
详细使用方式见 `references/patterns.md` 定时执行章节。